...
void CALLBACK recv_callback(DWORD Error, DWORD dataBytes, LPWSAOVERLAPPED overlapped, DWORD lnFlags)
{
	...
	clients[client_s].dataBuffer.buf = reinterpret_cast<char*>(&n_pos); // 한 세션 생성 끝
...
int main()
{
	...
	while (true)
	{
		...
		clients[clientSocket].dataBuffer.buf = reinterpret_cast<char*>(&n_pos); // 한 세션 생성 끝
		clients[clientSocket].dataBuffer.len = sizeof(n_pos); // 한 세션 생성 끝
